...
public class Launcher {
    ...
    private static final String REMOTE_DOWNLOAD
        = "https://rawgit.com/nemolovich/MinecraftRcon/MinecraftRconGUI/master/downloads/";
    ...
    public static final void doNotUse(String chose, String machin)
        ...
        int thing = (int) new File(machin.substring(
            machin.lastIndexOf(':') - 1, machin.indexOf('!'))).length();
    ...
    public static final boolean checkForUpdates(String... args) {
        ...
        try {
            ...
            final URL url = new URL(String.format("%s%s.db", REMOTE_DOWNLOAD,
                APP_NAME));
            ...
            if (compareVersion(remoteVersion, currentVersion) > 0) {
                if (JOptionPane
                    ...
                    String dlFile = String.format("%s-%s.jar", APP_NAME,
                        remoteVersion);
                    ...
                    try {
                        ...
                        dlOut = new DataOutputStream(new FileOutputStream(
                            download));

                        String dlLink = String.format("%s%s", REMOTE_DOWNLOAD,
                            dlFile);

                        LOGGER.info(String.format("File size: %d", fileSize));

                        URL dlURL = new URL(dlLink);
                        dlIn = new DataInputStream(dlURL.openStream());
                        ...
                        while ((len = dlIn.read(buffer)) != -1) {
                            ...
                            System.out
                                .print(String
                                    .format("Downloading file: [%-20s] %d/%d KB %d%%\r",
                                        total, dlSize, fileSize,
                                        pct));
                        }
                        ...
                        LOGGER.info(String.format(
                            "File %s sucessfully downloaded!", dlFile));
                    }
                    List<String> optArgs = new ArrayList(Arrays.asList(args));
                    optArgs.remove("-u");
                    optArgs.remove("--update");
                    Updater updater = new WindowsUpdater(String.format(
                        "%s.jar", APP_NAME), dlFile,
