...
import fr.nemolovich.apps.minecraftrcon.gui.exceptions.BrowserException;
...
public class MainFrame extends javax.swing.JFrame {
    ...
    public MainFrame(final String host, final int port, final String password,
        ...
        TableModelManager.getWhiteListPlayersFrame().setUpdateTask(
    ...
    private List<String> parseServerCommands(String msg) {
        String content = StringUtils.parseColorString(msg);
    ...
    public static Map<String, String> parsePlayersWithIP(String response) {
        ...
        if (response.contains("\n")) {
            String resp = StringUtils.parseColorString(
    ...
    public static List<String> parseOPPlayers(String response) {
        ...
        if (response.contains(":")) {
            String resp = StringUtils.parseColorString(response.substring(
                response.indexOf(':') + 1));
                ...
                if (nbPages > 1) {
                    for (int i = 2; i <= nbPages; i++) {
                        commands.addAll(getBasicCommands(
                            StringUtils.parseColorString(getNextHelp(i)),
                            true));
                    }
                }
            }
    ...
    private void customInitComponents() {
            ...
            this.dynamicFrame, TableModelManager.getPlayersFrame().getTable()) {
                ...
                public void action(String playerName) {
                    ...
                    try {
                        ...
                        res = String.format("Can not send message to player '%s'",
                            playerName);
        ...
        Button promotePlayerButton = new Button("Promote",
            "Add the selected player to admin operator list");
        promotePlayerButton.addActionListener(new PlayerActionListener(
            this.dynamicFrame, TableModelManager.getPlayersFrame().getTable()) {
                ...
                public void action(String playerName) {
                    ...
                    String res;
                    try {
                        res = getRequestResponse(String.format("op %s", playerName));
                        write(res, Level.INFO, dynamicFrameHelpPane);
                    } catch (IOException ex) {
                        res = String.format("Can not promote player to admin '%s'",
                            playerName);
                        write(res, Level.ERROR, dynamicFrameHelpPane);
                        LOGGER.error(res, ex);
                    }
            ...
            this.dynamicFrame, TableModelManager.getPlayersFrame().getTable()) {
                ...
                public void action(String playerName) {
                    ...
                    try {
                        ...
                        res = String.format("Can not ban player '%s'",
                            playerName);
                        write(res, Level.ERROR, dynamicFrameHelpPane);
                        LOGGER.error(res, ex);
                    }
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (index != -1) {
                    ...
                    try {
                        ...
                        res = String.format("Can not pardon player '%s'",
                            playerName);
                        write(res, Level.ERROR, dynamicFrameHelpPane);
                        LOGGER.error(res, ex);
                    }
        ...
        deopPlayerButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                TableFrameModel framemodel
                    = TableModelManager.getWhiteListPlayersFrame();
                JXTable table = framemodel.getTable();
                int index = table.getSelectedRow();
                if (index != -1) {
                    String playerName
                    ...
                    String res;
                    try {
                        res = getRequestResponse(String.format("whitelist remove %s",
                            playerName));
                        if (framemodel.getUpdateTask() != null) {
                            framemodel.getModel().clear();
                            framemodel.getUpdateTask().execute();
                        ...
                        res = String.format("Can not remove player from white list '%s'",
                            playerName);
                        write(res, Level.ERROR, dynamicFrameHelpPane);
                        LOGGER.error(res, ex);
                    }
        ...
        TableModelManager.getWhiteListPlayersFrame().addButton(deopPlayerButton);
    }
    ...
    private void whiteListItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_whiteListItemActionPerformed
        this.updateDynamicFrame(TableModelManager.getWhiteListPlayersFrame(),
            true);
        ...
        if (help == null || help.isEmpty()) {
            ...
            if (help != null) {
                Matcher multiPage = SERVER_COMMAND_PAGES_PATTERN.matcher(
                    StringUtils.parseColorString(help.replaceAll("\\n",
                            "\\\\n")));
                if (multiPage.matches()) {
